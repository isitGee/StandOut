  ...
  return (
    <div className="container" style={{ paddingTop: "1.25rem", paddingBottom: "2.5rem" }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
        <div>
          <h1 style={{ fontSize: "1.5rem", fontWeight: 700, letterSpacing: "-0.02em" }}>Build your CV</h1>
          <p className="small muted">All data stays in your browser. Switch templates anytime without re-typing.</p>
        </div>
        <div style={{ display: "flex", gap: "0.6rem", alignItems: "center" }}>
          ...
          <button
            ...
            onClick={() => {
              if (confirm("Clear this CV and start fresh?")) {
                clearCV();
                location.reload();
              }
            }}
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1rem", alignItems: "center" }}>
        ...
        <span className="small muted" style={{ marginLeft: "auto", display: "inline-flex", gap: "0.4rem", alignItems: "center" }}>
          <span style={{ width: 7, height: 7, background: "var(--brand)", borderRadius: 999, display: "inline-block" }} />
        ...
        <div className="builder-editor" style={{ display: mobileTab === "preview" ? "none" : "block" }}>
          ...
          <div className="editor-header">
            ...
            <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
              <span className="small muted">{active + 1} of {STEPS.length}</span>
            </div>
          </div>
            ...
            <div style={{ display: "flex", gap: "0.5rem" }}>
              {active < STEPS.length - 1 ? (
                <button className="btn btn-primary" onClick={next} type="button">
                  Continue <span className="btn-arrow" aria-hidden="true">&rarr;</span>
                </button>
              ) : (
                <button className="btn btn-primary" onClick={() => { setMobileTab("preview"); window.scrollTo({ top: 0, behavior: "smooth" }); }} type="button">
                  View preview <span className="btn-arrow" aria-hidden="true">&rarr;</span>
                </button>
              )}
            </div>
          </div>
          ...
          <div style={{ padding: "0.85rem 1.2rem", background: "var(--surface-2)", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span className="small muted">Data saved locally in browser</span>
            <button
              className="link-btn"
              onClick={() => {
                navigator.clipboard?.writeText(JSON.stringify(cv, null, 2));
                notify("CV JSON copied to clipboard — keep it as a backup.", "success");
              }}
              type="button"
            >
              Copy backup JSON
